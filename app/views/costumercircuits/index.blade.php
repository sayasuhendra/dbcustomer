...
<p>{{ link_to_route('costumercircuits.create', 'Add new circuit customer') }}</p>
		...
		<tbody>
			@foreach ($costumercircuits as $costumercircuit)
				<tr>
					...
					<td> {{{ $costumercircuit->present()->namavendor }}} </td>
					<td> {{{ $costumercircuit->present()->mrcVendor }}} </td>
                    ...
                    <td>{{{ $costumercircuit->present()->lamaBerlangganan }}}</td>
